...
void OptionManager::showUsage() {
  llvm::errs()
      ...
      << "  --algorithm            Algorithm used to reduce a list, default: chiseldd, candidates: probdd, chiseldd, fastdd, simplifiedprobdd \n"
...
void OptionManager::handleOptions(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, optstring, long_options, 0)) != -1) {
    ...
    case 'X':
      ...
      break;
    case 'a':
      OptionManager::Algorithm = std::string(optarg);
      break;
    case 'i':
      // do nothing
      break;
    case 'A':
      OptionManager::ComplementOnly = true;
      break;
    case 'B':
      OptionManager::Onepass = true;
      break;
    case 'E':
