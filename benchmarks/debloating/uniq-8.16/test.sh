...
function desired_run() {
  temp1=$({ timeout $TIMEOUT $REDUCED_BIN $1 $BENCHMARK_DIR/data.txt; } 2>&1 || exit 1)
  temp2=$({ $ORIGIN_BIN $1 $BENCHMARK_DIR/data.txt; } 2>&1)
  diff -q <(echo $temp1) <(echo $temp2) >&/dev/null || exit 1
  temp1=$({ timeout $TIMEOUT $REDUCED_BIN $1 $BENCHMARK_DIR/input; } 2>&1 || exit 1)
  temp2=$({ $ORIGIN_BIN $1 $BENCHMARK_DIR/input; })
  diff -q <(echo $temp1) <(echo $temp2) >&/dev/null || exit 1
  return 0
}
...
function desired_disaster_run() {
  { timeout $TIMEOUT $REDUCED_BIN $1 $BENCHMARK_DIR/data.txt; } >&$LOG
