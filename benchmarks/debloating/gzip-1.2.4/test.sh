...
function desired() {
  # -c
  $ORIGIN_BIN -c < $BENCHMARK_DIR/references/sample1.ref >sample1.gz 2>/dev/null
  { timeout $TIMEOUT $REDUCED_BIN -c < $BENCHMARK_DIR/references/sample1.ref >sample1.rb2; } 2>$LOG
  crash $? && exit 1
  cmp sample1.gz sample1.rb2 >&/dev/null || exit 1
  # -d
  { timeout $TIMEOUT $REDUCED_BIN -d <sample1.gz >sample1.tst; } 2>$LOG
  crash $? && exit 1
  cmp sample1.tst $BENCHMARK_DIR/references/sample1.ref >&/dev/null || exit 1
  # -f
  echo "1234" >foo
...
function undesired() {
  ...
  for opt in ${OPT1[@]}; do
    ...
    diff -q $BENCHMARK_DIR/references/side1 log2 >&/dev/null || exit 1
    rm -rf log*
  done
  ...
  for opt in ${OPT2[@]}; do
    ...
    diff -q $BENCHMARK_DIR/references/side2 log2 >&/dev/null || exit 1
    rm -rf log*
  done
...
#  done
  ...
  for opt in ${OPT4[@]}; do
    { timeout $TIMEOUT $REDUCED_BIN $opt; } &>$LOG
    diff -q $BENCHMARK_DIR/references/side4 $LOG >&/dev/null || exit 1
    rm -rf log*
  done
